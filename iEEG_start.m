...
for subji = 1:length(allsubs)
    ...
    EEG = pop_epoch( EEG, {}, [-4 8], 'newname', 'verbose', 'no', 'Continuous EEG Data epochs', 'epochinfo', 'yes');
...
for subji = 1:length(allsubs)
    ...
    %select electrodes at particular regions 
    chansLab = {EEG.chanlocs.fsLabel}';
    totalNofElec(subji,:) = length(chansLab);
    selChans = contains(chansLab, 'Amygdala');
