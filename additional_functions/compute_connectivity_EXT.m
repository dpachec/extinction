                ...
                for combi = 1:length(ids2comp(:, 1))
                    ...
                    counti = 1; 
                     for timei = toi
                        ...
                        amyD = squeeze(data(ids2comp(combi, 1), timei, :));
                        hpcD = squeeze(data(ids2comp(combi, 2), timei, :));

                        
                       hilbAMY = hilbert(amyD);
                       hilbHPC = hilbert(hpcD);
                       cdd = hilbAMY.* conj(hilbHPC);% cross-spectral density
                       %cdd = cdd(toi);
                       cdi = imag(cdd);
                       %PLV_M2(triali, combi,  :) = abs(mean(exp(1i*angle(cdd)))); % note: equivalent to PLV_SI(combi, chanj, :) = abs(mean(exp(1i*(diffPha)), 2)); %PLV
                       %PLI_M2(triali, combi,  :) = abs(mean(sign(imag(cdd))));
                       WPLI(combi, counti) = abs( mean( abs(cdi).*sign(cdi)))./mean(abs(cdi));% weighted phase-lag index (eq. 8 in Vink et al. NeuroImage 2011)

                       %diffPha = amyD - hpcD; 
                        
                           
                       %PLV_M1(combi, counti) = abs(mean(exp(1i*(diffPha)))); % just to test, works the same 
                       %PLI_M1(combi, counti) = abs(mean(sign(imag(exp(1i*diffPha))))); % just to test, works the same 
