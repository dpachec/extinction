...
for subji=1:1 %:length(allsubs)
    ...
    clearvars -except allsubs paths subji x_all x_nous y_all y_nous
    sel_sub=allsubs{subji};
    info_file=strcat(paths.trlinfo,sel_sub,'_trlinfo');
    load(info_file)  
    ...
    figure
    for condi=1:3 
        cond{condi}=trlinfo(trlinfo(:,6)==condi,:); % 6 = condition
        subplot(2,3,condi)
        hold on 
        y=cond{condi}(:,7); %7 = response
        ...
        if condi<=2
            ind_us=find(cond{condi}(:,9)==1); %us 0/1 (y/n)
            scatter(ind_us,ones(size(ind_us)),'r','o')
        end 
        title([sel_sub, ' ', conditions{condi}], 'Interpreter', 'none')
   end
